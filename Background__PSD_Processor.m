...
This script processes a collection of Background waveform signals by:
...
classification models as non-earthquake examples
...
for i = 1:length(eventNames)
    ...
    try
        ...
        % Since no metadata present, skip that part
        ...
        % Prepare event PSD struct without metadata (since none available)
        eventPSD = struct();
...
% --- Save Result ---
outputPath = fullfile(pwd, 'PSD_Windows_Background_100Hz.mat');
save(outputPath, 'psdResults');
fprintf('Saved PSD windows for background data to %s\n', outputPath);
